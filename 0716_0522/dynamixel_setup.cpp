...
void control::moveToPos(float x, float y, float z, float theta, int RL) {
    ...
    x = fabs(x + initialXPos);
    y = fabs(y + initialYPos); // y>0 left, y<0
    z = fabs(initialZPos - z);
...
bool control::leftMovement(float x, float y, float z, float theta, bool stop){ 
  // left y>0
    if (stop){
        ...
        moveToPos(x, -y, z, theta, 0);  // right
        moveToPos(x, 0, 0, theta, 0);  // right
    }else{
        moveToPos(0, y, z, theta, 0);  // left
        moveToPos(x, y, 0, theta, 0);  // left
        // delay(5000);
    }
    return true; 
}
...
bool control::rightMovement(float x, float y, float z, float theta, bool stop){
  // right y<0
    if (stop){
        moveToPos(x, y, z, theta, 6);  // right
        moveToPos(x, 0, 0, theta, 6);  // right
    }else{
        moveToPos(0, -y, z, theta, 6);  // right
        // delay(5000);
        moveToPos(x, -y, 0, theta, 6);  // right
        // delay(5000);
    }
    return true; 
}

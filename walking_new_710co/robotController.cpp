...
bool linearCoordinateMovement(float startX, float startY, float startZ, float targetX, float targetY, float targetZ) {
    const char *log;
    int steps = 1000; // 與原 linearMovement 的步數一致

    for (int i = 0; i < steps; i++) {
        // 使用與原 linearMovement 相同的插值公式
        float t = 0.5 * (1 - cos(M_PI * i / (steps - 1))); // 平滑插值
        float currentX = startX + (targetX - startX) * t;
        float currentY = startY + (targetY - startY) * t;
        float currentZ = startZ + (targetZ - startZ) * t;

        // 每次插值後調用 moveToPos
        if (!moveToPos(currentX, currentY, currentZ)) {
            Serial.println("Failed to move to interpolated position!");
            return false;
        }
    }
...
bool leftMovement(float x, float y, float z, bool stop) {
    Serial.print("Left Movement: x="); Serial.print(x);
    Serial.print(", y="); Serial.print(y);
    Serial.print(", z="); Serial.println(z);

    float oneStep = stop ? x : x / 2; // stop 時走完整步，否則走半步
    float startX = 0, startY = 0, startZ = initialZPos; // 假設起點為初始位置 (0, 0, initialZPos)

    if (stop) {
        // 停止時，左腳走半步後回到 (x, 0, 0)
        if (!linearCoordinateMovement(startX, startY, startZ, oneStep, -y, z)) {
            return false;
        }
        if (!linearCoordinateMovement(oneStep, -y, z, oneStep, 0, 0)) {
            return false;
        }
    } else {
        // 正常步行，左腳走半步到 (x/2, -y, z)，然後到 (x/2, -y, 0)
        if (!linearCoordinateMovement(startX, startY, startZ, oneStep, -y, z)) {
            return false;
        }
        if (!linearCoordinateMovement(oneStep, -y, z, oneStep, -y, 0)) {
            return false;
        }
    }
    return true;
...
bool rightMovement(float x, float y, float z, bool stop) {
    Serial.print("Right Movement: x="); Serial.print(x);
    Serial.print(", y="); Serial.print(y);
    Serial.print(", z="); Serial.println(z);
    ...
    if (stop) {
        ...
        // 正常步行，右腳走半步到 (x/2, y, z)，然後到 (x/2, y, 0)
        if (!linearCoordinateMovement(startX, startY, startZ, oneStep, y, z)) {
            return false;
        }
        if (!linearCoordinateMovement(oneStep, y, z, oneStep, y, 0)) {

...
float compensation = 5;

// y>0 right leg, y<0 left leg
//================================================================================================================
...
bool initializePosition() {
    ...
    IKAngles legAngles = calculateInverseKinematics(0, initialYPos, initialZPos);
...
bool moveToPos(float x, float y, float z, float thetaValue) {
    ...
    IKAngles initialLegAngles = calculateInverseKinematics(0, initialYPos, initialZPos);

    int32_t targetPositions[num_actuators];

    if (y > 0) { // right leg
        if (z == initialZPos){ // COM 在中間
            IKAngles initialLegAnglesY0 = calculateInverseKinematics(-x, 0, initialZPos); // 左腳向後推
            ...
            targetPositions[3] = convertRadianToPosition(M_PI + targetLegAngles.theta_hr) + compensation; // left_hip_roll
            targetPositions[4] = 2048; // left_hip_yaw
            targetPositions[5] = convertRadianToPosition(M_PI + targetLegAngles.theta_ar); // right_ankle_roll
            targetPositions[6] = convertRadianToPosition(-(targetLegAngles.theta_ap - M_PI)); // right_ankle_pitch
            targetPositions[7] = convertRadianToPosition(targetLegAngles.theta_hp + M_PI); // right_hip_pitch
            targetPositions[8] = convertRadianToPosition(M_PI + targetLegAngles.theta_hr) - compensation; // right_hip_roll
            targetPositions[9] = 2048; // right_hip_yaw
        }
    } else if (y < 0) {
        if (z == initialZPos){
            ...
            // rightPreviousPos 用於切換COM
            IKAngles rightPreviousPos = calculateInverseKinematics(x, y, initialZPos); // Keep the right foot in the previous position
            targetPositions[0] = convertRadianToPosition(M_PI + targetLegAngles.theta_ar) - compensation; // left_ankle_roll
            targetPositions[1] = convertRadianToPosition(targetLegAngles.theta_ap - M_PI); // left_ankle_pitch
            targetPositions[2] = convertRadianToPosition(-(targetLegAngles.theta_hp + M_PI)); // left_hip_pitch
            targetPositions[3] = convertRadianToPosition(M_PI + targetLegAngles.theta_hr) + compensation; // left_hip_roll ////////////////
            targetPositions[4] = initialLegPositions[4]; // left_hip_yaw
    ...
    } else if (y == initialYPos && z == initialZPos){ // stop
...
bool leftMovement(float x, float y, float z, float thetaValue, bool stop){
    ...
    if (stop){
        ...
        moveToPos(x, -y, z, thetaValue);  // left
        moveToPos(x, 0, 0, 0);  // left
    }else{
        moveToPos(0, -y, z, thetaValue);  // left
        moveToPos(x, -y, 0, thetaValue);  // left
        // delay(5000);
    }
    return true;
}

bool rightMovement(float x, float y, float z, float thetaValue, bool stop){
    ...
    if (stop){
        ...
        moveToPos(x, y, z, thetaValue);  // right
        moveToPos(x, 0, 0, 0);  // right
    }else{
        moveToPos(0, y, z, thetaValue);  // right
        // delay(5000);
        moveToPos(x, y, 0, thetaValue);  // right
        // delay(5000);
    }
return true;
}
...
bool walking(float x, float y, float z, float theta, bool stop) {
    ...
    if (stop) {
        ...
        if (currentStep == 1 || (currentStep == 2 && y > 0)) {
            ...
            rightMovement(oneStep, y, z, thetaValue, true);
        }
    ...
    if (currentStep == 1) {
        ...
        rightMovement(oneStep, y, z, thetaValue, false);
        currentStep = 2;
        return false; // next circle
    } else if (currentStep == 2) {
        Serial.println("Left");
        leftMovement(oneStep, y, z, thetaValue, false);
        // delay(10000);

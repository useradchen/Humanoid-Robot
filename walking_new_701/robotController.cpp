...
int steps = 10; // linearMovement

static int currentStep = 0; // 當前步數 (0 表示未開始，1 表示第一步，2 表示右腳循環，3 表示左腳循環)

static float currentX = 0, currentY = 0, currentZ = 0; // 當前使用的數值

static bool shouldRestart = false; // 標記是否需要重啟
//================================================================================================================
...
bool walking(float x, float y, float z) {
    ...
    if (currentStep == 0) {
        ...
        // 第一步：右腳
        Serial.println("Step 1: Right leg forward");
        ...
        // 主循環：右腳-左腳交替
        Serial.println("Right leg cycle");
        moveToPos(x, y, z);  // 右腳抬腳
        moveToPos(x, y, 0);  // 右腳放下

        Serial.println("Left leg cycle");
        moveToPos(x, -y, z); // 左腳抬腳
        moveToPos(x, -y, 0); // 左腳放下
